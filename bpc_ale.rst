...
* Select the useful data by automatically evalutating the median (in the time
  domain) of every subscan
* Build the bandpass curve by median-combining all the selected data
* Fit the bandpass curve
* Normalize the fit at peak value
* Divide each selected raw data by the normalized curve, create a new
  "corrected" data table and append it to the original file
...
.. image:: raw_corr.png
		...
		Because of the type of operations performed by this tool, data will be
		deformed, so this tool is not intended for quantitative measurements but only to facilitate the flagging operations of the data.
